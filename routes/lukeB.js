...
/* PLACES */
/* COMMENTS */
router.get("/add-comment",requiresLogin,function(req,res) {
    var data = req.query;
...
router.get("/edit-comment",requiresLogin,function(req,res){
    var data = req.query;
    if(data.id) {
...
router.get("/remove-comment",requiresLogin,function(req,res){
   var data = req.query;
    if(data.id) {
        remove(CommentModel,data.id);
    }else{
        res.status(200).json({error: "Missing id"});
    }
});
router.get("/vote-comment",requiresLogin,function(req,res){
    var data= req.query;
    if(data.id) {
        if(data.vote!=null) {
            CommentModel.findOne({id: data.id}, function (err, doc) {
...
router.get("/upvote-comment",requiresLogin,function(req,res){
    var data= req.query;
    if(data.id) {
        CommentModel.findOne({id: data.id}, function (err, doc) {
            if (err)throw err;
...
router.get("/downvote-comment",requiresLogin,function(req,res){
    var data= req.query;
    if(data.id) {
        CommentModel.findOne({id: data.id}, function (err, doc) {
            if (err)throw err;
